...
- R2. The four protected datasets remain untouched by local setup, reset, tests, and acceptance; test-only compatibility rows may exist only in a guarded local database.
...
- R6. One click on Sign In completes the Privy browser flow and immediately updates the persistent account control without a second click. Provider cancellation, verification failure, local-session failure, expiry, and renewal failure surface a recoverable account-level state instead of a silent no-op.
...
- R8. The account control shows an address-derived avatar and resolved identity. Its menu shows Profile only when the account owns a Regent, then Settings and Log Out in that order.
- R9. The fixed desktop/tablet shell and full-feature mobile shell expose Formation, Autolaunch, Techtree, and Regents Labs through one app selector and app-local contextual navigation.
...
- R11. The approved homepage, structural material, background, Settings-owned System/Light/Dark theme control, square geometry, voxel, reduced-motion, keyboard, no-JavaScript, responsive, and 200%-zoom behavior is packaged from clean files rather than depending on the mixed shared checkout.
...
- R13. Regents Labs provides Overview, Stake, Redeem, and conditional Profile destinations under the persistent shell.
...
- R23. Notebook execution is user-started and WASM-based on a separate opaque local origin. It runs in a restrictive iframe sandbox with no ambient credentials or storage, strict message source/origin checks, network denied by default, bounded time and memory, locally allowlisted runtime assets, hash verification, and honest loading, ready, failed, timeout, resource-limit, and unsupported states.
...
- R28. Agent authentication uses shared SIWA verification and a local agent signer through a separate contract and transport boundary from Privy browser sessions. Shared SIWA remains the sole nonce, signed-envelope, and replay authority; Ash Platform consumes a verified receipt and maps it to a local actor instead of creating competing replay state. Every write then reloads current product-local agent/account status and action-specific ownership or workflow permission; suspended, revoked, unpaired, wrong-owner, or otherwise ineligible actors are denied.
...
- R31. An agent evidence reaction is admitted only after verified delivery of the full current node payload. A product-owned delivery record binds agent, audience, challenge, stable node address, immutable payload version and hash, issued time, and expiry. It may be reused by that agent only for the same unchanged node version until expiry; a changed version requires a new delivery record. It does not reuse SIWA replay secrets.
- R32. The one mutable agent reaction per agent and stable node address uses exactly: Evidence checks out, Result reproduced, Useful, Needs evidence, Contradicted, or Could not reproduce. It stores the reviewed payload hash and remains historical provenance when the payload later changes.
...
- R36. A signed-in human can create and validate a launch draft, optionally connect verified X, Farcaster, ENS, and World reputation, review the launch, and prepare the admitted auction actions.
...
- R44. Customers fund one prepaid balance before Sprite or AI work. The canonical ledger, not a shadow balance, owns funding, reservations, settlement, release, expiry, refunds, and reconciliation.
- R45. Work reserves credit before provider exposure. New work is denied when it cannot be reserved; already-started work may finish only inside its reservation; the Sprite pauses near zero until top-up.
- R46. Stripe top-up and refund flows verify signatures over untouched raw bytes, timestamp tolerance, expected account and environment before parsing; persist one inbox identity; then authoritatively retrieve and match customer, owner, currency, amount, mode, and status. They converge under invalid-first delivery, duplicates, reordering, timeout, exhausted retry, and crash recovery. Test adapters and Stripe test mode never imply production entitlement.
- R47. Daily maximum, persisted auto-recharge consent and revocation, threshold, amount, currency, cap, reservation expiry, delayed usage, refunds limited to unspent refundable credit, small exceptional exposure, provider failure, and user-visible reconciliation are bounded, monitored, and durably evidenced.
...
- F1. Clean local start
  ...
  - **Actors:** A6.
  - **Steps:** Run the documented setup; guard the database target; install exact locks; migrate and seed deterministic fixtures; start the application and local notebook origin.
...
- F7. Formation and prepaid runtime
  ...
  - **Outcome:** Hosted work cannot begin unbacked, and provider failures converge into visible account state.
  - **Covered by:** R41-R48.
...
- Real Privy is required for the manual first-click acceptance. Stripe test mode, live Sprites, public-chain reads, and user-signed Base canaries are separate controlled checks; deterministic local providers remain the automated completion gate.
...
- Automated acceptance selects fake providers explicitly and enforces an egress tripwire. Production clients fail closed without required secrets, and tests never log tokens, cookies, signed envelopes, webhook bodies, wallet signatures, or sensitive prepared-action data.
...
- KTD6. **Practical prepaid through one ledger.** Funding, reservation, settlement, release, expiry, and reconciliation share one canonical Ash accounting model. Provider adapters never own a shadow balance.
...
- KTD12. **Immutable accounting history, verified projections.** Funding, reservation, settlement, release, expiry, refund, and reconciliation events reconstruct the balance. All equation terms are non-negative magnitudes: gross funded credit equals available plus active reserved plus consumed plus refunded. Settlement moves reserved to consumed; release or expiry moves reserved to available; refund moves available to refunded. Mutable totals are verified projections only.
- KTD13. **One constrained comment table, named target interfaces.** A comment has nullable `techtree_node_id`, `autolaunch_auction_id`, and `autolaunch_token_id` foreign keys with a database CHECK requiring exactly one. Named domain interfaces resolve and authorize each target type; no generic unvalidated type/id pair or application-only referential integrity is admitted.
...
flowchart TB
  ...
  U4 --> U3["U3 shell identity package"]
...
- **Data lifecycle:** Every persistence unit migrates from the previously populated local schema, preserves existing rows, inspects its generated migration and snapshot diff, rehearses rollback or recovery on a disposable local copy, and reapplies successfully. Acceptance-created records carry a run marker and reset returns every owned table to the post-setup baseline without orphan references, active reservations, unsettled provider events, runnable jobs, provider-fake objects, or seed drift.
...
- **Sprites billing cutoff uncertainty:** Local behavior can prove reservation and Platform admission, but any provider billing-stop guarantee stays bounded by documented vendor behavior. Surface any remaining maximum-exposure assumption instead of inventing a hard cutoff.
...
| U3 | Package shell, Privy, profile, and Settings | shell/session/account files, `app.ts`, `app.css` | U4 |
...
- **Approach:** Apply the founder-approved old-platform/Ash-platform ownership decision to the active sources before product writers resume. Name canonical HTTP, CLI, runtime, and prepared-chain-action owners. Pin the exact `elixir-utils` Privy commit. Record immutable bases, declared paths and hunks, collisions, generated residue, and evidence; establish one integration custodian and clean candidates from `747c953e83343107ce6100c65252514fa52cac95`; preserve the two sealed unrelated CLI generated diffs.
...
- **Approach:** Reconstruct the reviewed homepage from clean `747c953`, use the founder SVG byte-for-byte, and import only `HomeHero` and homepage CSS. Keep shell, product, Privy, and wallet hooks outside this unit.
- **Test scenarios:** No-JavaScript readability; dark/light pinned-dark asset equality; hook settle/cancel/destroy; primary action hover/focus contrast; narrow mobile, tablet, desktop, effective 200% zoom, keyboard, and reduced motion.
...
- **Files:** `contracts/api-contract.openapiv3.yaml`; `lib/ash_platform/accounts*`; `lib/ash_platform/access_context*`; `lib/ash_platform/privy.ex`; browser session controllers/plugs; `lib/ash_platform_web/live/session.ex`; `lib/ash_platform_web/live/shell_live.ex`; `lib/ash_platform_web/components/shell.ex`; shell/background/account components; `assets/js/privy_bridge.tsx`; shell/theme/motion hooks; shell/material/profile CSS; exact shared entrypoint imports; route handoff artifact and focused tests.
- **Approach:** Start from committed auth and Settings. Admit the reviewed account avatar/menu/profile presentation, server-supplied profile target, canonical read-only Regent ownership projection, shell material, product selector, mobile menu, theme, navigation, and motion through one integration owner. Verify only Privy access tokens with the expected issuer, audience, subject, application, and expiry; rotate bounded secure sessions on sign-in/renewal and invalidate them server-side on logout. Keep product data panels as honest route states until their units land.
- **Test scenarios:** Real hard-refresh first-click Privy; identity-token rejection; wrong issuer/application/audience; cancellation, provider failure, local-session failure, expiry, renewal, reconnect, logout, CSRF, pre-auth session reuse, wrong signer, untrusted browser claims; account with and without Regent; anonymous patch to Settings; persisted theme from Settings; desktop/mobile parity; rapid app switching; Back/Forward; retryable destination failure; no-JavaScript; keyboard/reduced motion/200% zoom; captured-log redaction.
...
- **Files:** `.tool-versions` or the selected checked-in toolchain manifest; `package.json` engines/package-manager metadata; `mix.exs`; `config/config.exs`; `config/dev.exs`; `config/test.exs`; `lib/mix/tasks/ash_platform.setup_local.ex`; `lib/mix/tasks/ash_platform.reset_local.ex`; `lib/ash_platform/local_database_fixture.ex`; test support; `.env.example`; `docs/local-founder-acceptance.md`; focused setup/reset safety tests.
- **Approach:** Preflight the supported host/toolchain before mutation. Extend the guarded local fixture pattern to create a unique `ash_platform_acceptance_<run_id>` database only on the approved loopback server, migrate/seed deterministic data, capture its baseline, build assets, and mark acceptance-created records. Teardown verifies owned-table baseline and no-residue invariants, then drops only that exact run database; repeated teardown is idempotent. Enforce a non-loopback egress tripwire. Never read `.env`, `.env.local`, or `.envrc` during implementation or tests.
- **Test scenarios:** Unsupported host/tool version; fresh and colliding run name; unexpected host, role, database, schema, or environment; configured production URLs present but unused; missing variables; repeated/interrupted setup; repeated teardown; unrelated databases preserved; seed hashes preserved until drop; attempts to create/alter/write each protected dataset name; generated outputs trap-cleaned.
...
- **Approach:** Remove generated notebook residue, reconcile only current owned boundary assertions, finish the active independent protected review, commit the candidate, and import with an old-value guard. Preserve the old page behavior while using the shared wallet envelope and real bounded ENS resolver.
...
- **Files:** Ash Platform `contracts/chain-contracts.yaml` Redeem entries and generated Animata ABI/manifest; `lib/ash_platform/redemption*`; `lib/ash_platform/wallet_actions/redemption_abi.ex`; `lib/ash_platform_web/live/redeem_live.ex`; `assets/js/wallet_actions/redemption.ts`; `assets/css/pages/redeem.css`; read-only holdings adapter; fake chain client; focused tests and docs.
- **Approach:** Reuse the accepted wallet envelope, confirmation, and RPC interfaces. Reproduce the visible eligibility, approval, redeem, and claim journey. Keep read-only holdings convenience separate from signing authority.
- **Test scenarios:** Eligible/ineligible holdings; already redeemed; NFT approval bound to exact token ID and spender; token approval bound to exact required amount and spender; unlimited/wrong approval rejected; redeem; claim; user cancellation; wallet/chain mismatch; expiry; pending/reverted/replaced/reorganized/duplicate receipt; mobile wallet handoff and return; stale holdings; provider failure; reread mismatch.
...
- **Approach:** Make the five roots canonical Ash data, preserve one route for Map/List state, implement a real starmap from node relationships, and establish the stable-address/version invariant before agent publishing: immutable globally unique address, one same-tree parent for each non-root, durable publisher/receipt, immutable content-addressed versions, atomic current-version switch, and retrievable prior bytes. Serve only hash-verified notebook artifacts on R23's separate opaque origin using sandboxed `allow-scripts` without `allow-same-origin`, restrictive CSP, no credentials/storage/network, strict postMessage checks, and bounded workers. Keep browser node creation absent.
- **Test scenarios:** Root order; reserved slug; direct/deep links; tree-name mode preservation; icon-forced mode; cancellable panel transition; unknown/empty tree; cross-tree/double parent rejection; duplicate address; version immutability and atomic switch; retrievable prior evidence; artifact missing/hash mismatch/origin mismatch; credential/storage/network/message spoof attempts; infinite loop/memory pressure; notebook loading/ready/failure/timeout/resource limit; touch/keyboard/reduced motion.
...
- **Approach:** Use KTD13's constrained Ash Comment resource and atomic private deletion audit, with exactly one existing target and exactly one immutable human author rail in this unit. Initial target records are never hard-deleted; visibility changes are rechecked for every query and real-time reread. Scope idempotency to principal, target, and input digest. Add a separate human reaction resource only for Techtree, exclude deleted comments from public counts, notify after commit, and reread through Ash rather than trusting event payloads. Deny agent writes until U16.
...
- **Files:** Shared service contract only if shared SIWA itself changes; Ash `contracts/api-contract.openapiv3.yaml`; agent security scheme/controllers/plugs; verified shared-SIWA receipt mapping and local actor/status projection; agent extension to the U8 comment action/policy; deterministic CLI OpenAPI projection; minimal agent auth/comment CLI YAML, generated bindings, tests, and fixtures.
- **Approach:** Keep shared SIWA as nonce, signature, and replay authority. Verify the exact signed envelope before replay consumption, map the receipt to one local actor, reload current product eligibility and target permission for each write, and expose separate SIWA-only comment operations. No browser cookie, product nonce, Techtree publication, notebook, evidence reaction, runtime, or value action enters this unit.
- **Test scenarios:** Invalid signature followed by valid replay-identity use; changed method/path/header/audience/raw bytes; replay; suspended/revoked/unpaired/wrong-owner actor; browser cookie on agent route; SIWA on browser route; agent comment create/delete idempotency scoped to principal/target/body; hidden target; reset.
...
- **Approach:** Reuse U16's verified actor and action policy, then add public reads and separately signed Techtree writes. Publish through Ash interfaces. Notebook intake accepts only canonical UTF-8 Python source with `text/x-python`, exact-byte content hashing, server-generated storage keys, no archive/compression, a 5 MiB artifact limit, and a 100 MiB per-principal local-beta quota. Use R31's product-owned full-payload delivery record, reusable only by the same agent for the unchanged version until expiry, before an evidence reaction. Hard-cut old numeric IDs, voting, and paid-payload shapes.
- **Test scenarios:** Wrong audience/agent/node/version/hash; expired or misused delivery record; idempotent publication scoped to principal/parent/body; payload partially read; payload changed after delivery; notebook MIME/UTF-8/hash/path/size/quota failure; one reaction per stable address; reaction update/remove; retrievable old version provenance; structured CLI failures and reset.
...
- **Approach:** Make auctions, tokens, and launch drafts canonical Ash resources. Use one profile-owned reputation projection for optional verified X, Farcaster, ENS, and World signals. Integrate the U8 comment kernel with canonical Autolaunch targets, newest-first and no reaction resource. Add browse, draft validation, action preparation, and agent comments without granting agent signing authority.
- **Test scenarios:** Recent/featured/top/graduated queries; honest empty/error states; draft validation and idempotency; optional/expired/mismatched reputation; public/restricted comment visibility; author/admin deletion; agent preparation without signing; stale record/action state.
...
- **Approach:** Reconstruct the reviewed one-Regent and profile work on the accepted auth checkpoint. Establish one durable Regent/runtime claim, require U17 prepaid admission before any provider call, persist provider idempotency identity, and reconcile provider-success/database-failure. Expose bounded status and pause/resume/health, and make Hermes profiles, skills, chat, and plugin state real. Keep provider HTTP behind a bounded adapter and do not revive public/private messaging.
- **Test scenarios:** No Regent/one Regent; duplicate create; truly concurrent provision; credit admission denied; provider timeout/invalid response/timeout-after-success; provider success before DB failure; one external Sprite and one row; refresh freshness; pause/resume/health; Hermes profile and skill changes; chat/plugin status; owner/other/SIWA-agent authority; mobile panels; provider-free local acceptance.
...
- **Goal:** Extend U17's funding/reservation kernel with provider-backed funding, refunds, auto-recharge, full reconciliation, and the founder-visible Billing experience.
...
- **Approach:** Preserve U17's immutable history, idempotency identities, and ledger equation. Persist a verified provider inbox identity before processing, retrieve authoritative provider state outside the accounting transaction, then process through retryable jobs. Add refunds, auto-recharge, delayed-usage reconciliation, and visible exceptional exposure. The Billing panel has explicit flows for initial funding, available/reserved/consumed/refunded balance, top-up review, refund request, auto-recharge consent/edit/revoke, reconciliation, paused service, and provider failure; each exposes entry, review, pending, success, and recoverable-error states without claiming provider success early.
- **Test scenarios:** Invalid-first and valid-redelivery webhook; raw-byte/signature/timestamp/account/environment mismatch; funding replay/wrong owner/customer/currency/amount/status; independent-transaction reservation race; settlement-versus-expiry; refund-versus-consumption; auto-recharge-versus-revocation and daily cap; partial/final settlement; release/expiry; delayed usage; provider timeout after success; retry/exhaustion/reordering; insufficient credit; final-window clamp; pause-near-zero; no new work while paused; reservation-bounded completion; invariant after every transition.
...
- **Requirements:** R1-R55.
- **Files:** Cross-repo acceptance runner; `docs/local-founder-acceptance.md`; final `SeanTasks.md` entries only for unavoidable founder credentials/device steps; evidence manifests and deterministic fixtures; no product behavior invented here.
- **Approach:** Start from clean pinned checkouts of Ash Platform, Regents CLI, and the admitted `elixir-utils` dependency. Create the unique guarded acceptance database, capture its post-setup baseline, assign one run marker to all acceptance-created records, then run app, notebook origin, browser journeys, fake protected providers, packaged CLI, and all gates. On success, forced failure, interruption, and a second complete run, tear down children in dependency-safe order; run reset twice against the still-live disposable database and compare every owned table with baseline after each reset; assert no orphan/active/inbox/job/provider residue; then drop only that exact run database, clean generated outputs, and verify final Git state. Compare requirement-by-requirement rather than relying on test counts. Replace stale README and route claims with current verified capabilities and commands.
...
- **Verification:** The acceptance command exits zero twice; every AE is evidenced; all children and temp paths are absent; both reset passes on each run restore the post-setup baseline before teardown; both unique run databases are absent afterward; all no-residue invariants hold; all three checkouts are clean; directives comparison has no unexplained gap; `SeanTasks.md` contains only actions the founder must personally perform.
...
| Budgets | `npm run test:budgets` | U2, U3, U7-U17 | Asset and behavior budgets pass |
...
| Migration rehearsal | Capability-owned migration/snapshot check from U4 | U3, U7-U13, U16, U17 | Populated prior schema migrates, preserves rows, recovery/rollback rehearsal succeeds, and reapply converges |
