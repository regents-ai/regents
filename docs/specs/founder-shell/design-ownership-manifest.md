...
`post_commit_source_digest` remains `PENDING_POST_COMMIT_SOURCE_DIGEST` until the immutable scaffold commit exists. Replacing it before that commit would invent an identity that cannot yet be verified.
