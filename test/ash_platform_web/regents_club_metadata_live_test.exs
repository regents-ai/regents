defmodule AshPlatformWeb.RegentsClubMetadataLiveTest do
  ...
  test "confirmation replaces the reviewed envelope with a freshly revalidated anchor", %{
    ...
    owner = account!("owner-fresh-confirm", [@owner])
    {:ok, view, _html} = mount(conn, owner)
    render_async(view)
    render_hook(view, "regents_club_metadata_active_wallet", %{"address" => @owner})
  ...
  test "flag, media, and preflight drift after review each refuse wallet handoff", %{conn: conn} do
    for {suffix, drift} <- [
      ...
      owner = account!("confirm-drift-#{suffix}", [@owner])
      {:ok, view, _html} = mount(conn, owner)
      render_async(view)
  ...
  test "confirmation revalidates the current session lease and linked owner", %{conn: conn} do
    owner = account!("confirm-linked-owner", [@owner])
    signed_conn = init_test_session(conn, %{human_account_id: owner.id})
    {:ok, view, _html} = live(signed_conn, "/regents-club/metadata-cutover")
    render_async(view)
    prepare_review(view, @attempt)

    assert {:ok, _lapsed} = Accounts.refresh_verified(owner, nil, [], actor: %System{})

    render_hook(view, "confirm_regents_club_metadata", %{"attempt_id" => @attempt})
    assert has_element?(view, "#account-control [data-account-target=sign-in]")
    refute_push_event(view, "regents-club-metadata:prepared", _payload)

    second_owner = account!("confirm-revoked-owner", [@owner])
    second_conn = init_test_session(build_conn(), %{human_account_id: second_owner.id})
  ...
  test "finality observation continues beyond forty polls and never reopens a send", %{conn: conn} do
    ...
    owner = account!("owner-delayed-finality", [@owner])
    {:ok, view, _html} = mount(conn, owner)
    render_async(view)
    prepare_review(view, @attempt)
    render_hook(view, "confirm_regents_club_metadata", %{"attempt_id" => @attempt})
  ...
  test "fresh handoff deadline becomes unknown without another wallet request", %{conn: conn} do
    ...
    owner = account!("owner-deadline", [@owner])
    {:ok, view, _html} = mount(conn, owner)
    render_async(view)
    prepare_review(view, @attempt)
    render_hook(view, "confirm_regents_club_metadata", %{"attempt_id" => @attempt})
  ...
  defp prepare_review(view, attempt_id) do
    render_hook(view, "regents_club_metadata_active_wallet", %{"address" => @owner})
