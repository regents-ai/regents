...
interface ISafeMessageHash {
    function getMessageHashForSafe(address safe, bytes calldata message)
        external
        view
        returns (bytes32);
}
...
contract AgentSafePolicyTest is Test {
    ...
    address internal constant PINNED_HANDLER = 0xfd0732Dc9E303f09fCEf3a7388Ad10A83459Ec99;
    ...
    function setUp() external {
        ...
        factory = PINNED_FACTORY;
        handler = PINNED_HANDLER;
        vm.etch(singleton, _safeRuntime());
        vm.etch(factory, _factoryRuntime());
        vm.etch(handler, _handlerRuntime());
        policy = new AgentSafePolicyMock();

        address[] memory owners = new address[](1);
        owners[0] = vm.addr(LAUNCHER_KEY);
        safe = _createSafe(owners, 1, 1);
        expected = _expectedState(safe);
    }

    function testLauncherOwnedTrueOneOfOne() external view {
        policy.validate(
            safe,
            CONTROLLER,
            RUNTIME,
            expected,
            DIGEST,
            _signatures(safe, DIGEST, _keys(LAUNCHER_KEY, 0))
        );
    }

    function testThresholdValidTwoOfThree() external {
        (address thresholdSafe, AgentSafePolicy.ExpectedState memory thresholdExpected) =
            _twoOfThreeSafe();
        policy.validate(
            thresholdSafe,
            CONTROLLER,
    ...
    function testRejectsOneSignatureAtTwoOfThree() external {
        (address thresholdSafe, AgentSafePolicy.ExpectedState memory thresholdExpected) =
            _twoOfThreeSafe();
        bytes memory signature = _signatures(thresholdSafe, DIGEST, _keys(LAUNCHER_KEY, 0));
        vm.expectRevert();
        policy.validate(thresholdSafe, CONTROLLER, RUNTIME, thresholdExpected, DIGEST, signature);
    }

    function testRejectsChangedOwner() external {
        expected.ownersHash = keccak256("changed-owner");
        _expectPolicyRevert(_validSignature());
    }

    function testRejectsChangedThreshold() external {
        expected.threshold = 2;
        _expectPolicyRevert(_validSignature());
    }

    function testRejectsChangedNonce() external {
        expected.nonce = 1;
        _expectPolicyRevert(_validSignature());
    }

    function testRejectsModule() external {
        bytes32 sentinelSlot = keccak256(abi.encode(SENTINEL, uint256(1)));
        vm.store(safe, sentinelSlot, bytes32(uint256(uint160(address(0xD00D)))));
        _expectPolicyRevert(_validSignature());
    }

    function testRejectsGuard() external {
        vm.store(safe, GUARD_STORAGE_SLOT, bytes32(uint256(uint160(address(0x600D)))));
        _expectPolicyRevert(_validSignature());
    }

    function testRejectsFallbackHandler() external {
        vm.store(safe, FALLBACK_HANDLER_STORAGE_SLOT, bytes32(uint256(uint160(address(0xFA11)))));
        _expectPolicyRevert(_validSignature());
    }

    function testRejectsSingleton() external {
        vm.store(safe, bytes32(0), bytes32(uint256(uint160(handler))));
        _expectPolicyRevert(hex"");
    }

    function testRejectsProxyRuntime() external {
        vm.etch(safe, hex"00");
        _expectPolicyRevert(hex"");
    }

    function testRejectsImplementationRuntime() external {
        vm.etch(singleton, hex"00");
        _expectPolicyRevert(hex"");
    }

    function testRejectsFactoryRuntime() external {
        vm.etch(factory, hex"00");
        _expectPolicyRevert(_validSignature());
    }

    function testRejectsFallbackHandlerRuntime() external {
        vm.etch(handler, hex"00");
        _expectPolicyRevert(hex"");
    }

    function testRejectsWrongEip1271Magic() external {
        bytes memory signature = _validSignature();
        bytes memory callData = abi.encodeCall(IERC1271.isValidSignature, (DIGEST, signature));
        vm.mockCall(safe, callData, abi.encode(bytes4(0xffffffff)));
        _expectPolicyRevert(signature);
    }

    function testRejectsRevertingEip1271() external {
        bytes memory signature = _validSignature();
        bytes memory callData = abi.encodeCall(IERC1271.isValidSignature, (DIGEST, signature));
        vm.mockCallRevert(safe, callData, abi.encode("revert"));
        _expectPolicyRevert(signature);
    }

    function testRejectsMalformedEip1271Return() external {
        bytes memory signature = _validSignature();
        bytes memory callData = abi.encodeCall(IERC1271.isValidSignature, (DIGEST, signature));
        vm.mockCall(safe, callData, hex"1626");
        _expectPolicyRevert(signature);
    }

    function testRejectsControllerOwner() external {
        address[] memory owners = new address[](1);
        owners[0] = CONTROLLER;
        address forbiddenSafe = _createSafe(owners, 1, 2);
        AgentSafePolicy.ExpectedState memory forbiddenExpected = _expectedState(forbiddenSafe);
        vm.expectRevert();
        policy.validate(forbiddenSafe, CONTROLLER, RUNTIME, forbiddenExpected, DIGEST, hex"");
    }

    function testRejectsRuntimeOwner() external {
        address[] memory owners = new address[](1);
        owners[0] = RUNTIME;
        address forbiddenSafe = _createSafe(owners, 1, 3);
        AgentSafePolicy.ExpectedState memory forbiddenExpected = _expectedState(forbiddenSafe);
        vm.expectRevert();
        policy.validate(forbiddenSafe, CONTROLLER, RUNTIME, forbiddenExpected, DIGEST, hex"");
    }
    ...
    function testCanonicalFamilyIsExactCertifiedTuple() external view {
        ...
        assertEq(
            factory.codehash, 0x50c3cdc4074750a7a974204a716c999edd37482f907608d960b2b025ee0b3317
        );
        assertEq(
            handler.codehash, 0x7c6007a5d711cea8dfd5d91f5940ec29c7f200fe511eb1fc1397b367af3c42f9
        );
        ...
        assertEq(
            bytes32(sourceCommit), bytes32(bytes20(hex"bf943f80fec5ac647159d26161446ac5d716a294"))
        );
        assertEq(
            bytes32(sourceTree), bytes32(bytes20(hex"dbbe8faa94445342975303ff4da1471cac2052d6"))
        );
        assertEq(
            bytes32(deploymentsCommit),
            bytes32(bytes20(hex"3274616f3e5e2ab1f2cba6aec8e1abb9971e61cb"))
        );
        assertEq(
            bytes32(deploymentsTree),
            bytes32(bytes20(hex"573a96fa5b7b6e5629d6a51d7bcd1705cd9d5faf"))
        );
        (
        ...
        assertEq(bytes32(createProxyWithNonce), bytes32(bytes4(0x1688f0b9)));
        assertEq(bytes32(setup), bytes32(bytes4(0xb63e800d)));
        assertEq(bytes32(excludedChainSpecific), bytes32(bytes4(0xec9e80bb)));
    ...
    function _twoOfThreeSafe()
        internal
        returns (address thresholdSafe, AgentSafePolicy.ExpectedState memory thresholdExpected)
    {
        address[] memory owners = new address[](3);
        owners[0] = vm.addr(LAUNCHER_KEY);
        owners[1] = vm.addr(OWNER_TWO_KEY);
        owners[2] = vm.addr(OWNER_THREE_KEY);
        thresholdSafe = _createSafe(owners, 2, 4);
        thresholdExpected = _expectedState(thresholdSafe);
    }

    function _createSafe(address[] memory owners, uint256 threshold, uint256 saltNonce)
        internal
        returns (address created)
    {
        ...
        created =
            IPinnedSafeFactory(factory).createProxyWithNonce(singleton, initializer, saltNonce);
    }

    function _expectedState(address target)
        internal
        view
        returns (AgentSafePolicy.ExpectedState memory state)
    {
        address[] memory owners = IAgentSafe(target).getOwners();
        state = AgentSafePolicy.ExpectedState({
            ownersHash: keccak256(abi.encode(owners)),
            ownerCount: owners.length,
            threshold: IAgentSafe(target).getThreshold(),
            nonce: IAgentSafe(target).nonce(),
            modulesHash: policy.emptyModulesHash(),
            guard: address(0)
        });
    }

    function _validSignature() internal view returns (bytes memory) {
        return _signatures(safe, DIGEST, _keys(LAUNCHER_KEY, 0));
    }

    function _expectPolicyRevert(bytes memory signature) internal {
        vm.expectRevert();
        policy.validate(safe, CONTROLLER, RUNTIME, expected, DIGEST, signature);
    }

    function _keys(uint256 first, uint256 second) internal pure returns (uint256[] memory keys) {
        keys = new uint256[](second == 0 ? 1 : 2);
        keys[0] = first;
        if (second != 0) keys[1] = second;
    }

    function _signatures(address target, bytes32 digest, uint256[] memory keys)
        internal
        view
        returns (bytes memory signatures)
    {
        bytes32 messageHash =
            ISafeMessageHash(handler).getMessageHashForSafe(target, abi.encode(digest));
        if (keys.length == 2 && vm.addr(keys[0]) > vm.addr(keys[1])) {
    ...
    function _safeRuntime() private pure returns (bytes memory) {
    ...
    function _factoryRuntime() private pure returns (bytes memory) {
    ...
    function _handlerRuntime() private pure returns (bytes memory) {
