defmodule AshPlatformWeb.SharedProfileControllerTest do
  use AshPlatformWeb.ConnCase, async: false

  setup_all do
    database = AshPlatform.Repo.config()[:database]

    unless database == "ash_platform" <> System.fetch_env!("MIX_TEST_PARTITION") <> "_test",
      do: raise("Profile tests require the prepared disposable database")

    RegentIdentity.Migrator.up(AshPlatform.Repo)
    assert RegentIdentity.Migrator.up(AshPlatform.Repo) == []
    :ok
  end
  ...
  setup do
    ...
    Application.put_env(:ash_platform, :privy,
