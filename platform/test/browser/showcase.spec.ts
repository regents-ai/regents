...
test("Ash validation, utility outcomes, local database and disclosure fixtures work", async ({
  ...
  await page.locator("#showcase-step").getByRole("button", {name: "Edit", exact: true}).click()
  await expect(page.locator("#step-title")).toBeFocused()
  await page.locator("#step-title").fill("x")
  await page.getByRole("button", {name: "Save title", exact: true}).click()
  await expect(page.locator("#step-title")).toHaveAttribute("aria-invalid", "true")
  await expect(page.locator("#step-title")).toHaveValue("x")
  await page.locator("#step-title").fill("Updated local step")
  await page.getByRole("button", {name: "Save title", exact: true}).click()
  await expect(page.locator("#showcase-step")).toContainText("Updated local step")
  await page.locator("#showcase-step").getByRole("button", {name: "Edit", exact: true}).click()
  await page.locator("#step-title").fill("Discard this edit")
  await page.getByRole("button", {name: "Cancel", exact: true}).click()
  await expect(page.locator("#showcase-step")).not.toContainText("Discard this edit")

  await context.grantPermissions(["clipboard-read", "clipboard-write"])
  await page.locator("#showcase-facts [data-sc-copy]").click()
  await expect(page.locator("#showcase-facts [data-sc-copy]")).toHaveText("Copied")
